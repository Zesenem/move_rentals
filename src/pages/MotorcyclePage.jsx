    ...
    <div className="py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 mb-12">
        ...
        <div>
          ...
          <div className="mt-8">
            <a href={whatsappURL} target="_blank" rel="noopener noreferrer" className="w-full text-center bg-cloud text-phantom font-bold py-4 px-8 rounded-lg text-lg hover:brightness-95 transition-all duration-300 inline-flex justify-center items-center gap-3">
              <FaWhatsapp size={20} />
              Book via WhatsApp
            </a>
          </div>
          <div className="mt-10">
            ...
            <AccordionItem title="Optional Extras">
              <ul className="text-base space-y-3">
                <li className="p-3 bg-arsenic/50 rounded-md"><span className="font-semibold text-cloud">{bike.extras_specific.item}:</span><span className="block text-space mt-1">{bike.extras_specific.price}</span></li>
          ...
          <AccordionItem title="Included in Rental">
            ...
            <ul className="text-base space-y-2.5 text-steel">
              {commonData.included.map((item, index) => (
                <li key={index} className="flex items-center">
                  {iconMap[item.icon] || <FaCheckCircle className="text-green-400 mr-3 flex-shrink-0" />}
                  {item.item}
                </li>
              ))}
            </ul>
