...
function BookingWidget({ bike, selectedExtras }) {
    ...
    if (isLoading) {
        return (
            <div className="bg-arsenic p-6 rounded-lg border border-graphite/50 flex justify-center items-center h-96">
            ...
            <div className="mt-6 border-t border-graphite/50 pt-6">
                ...
                <div className="mt-6">
                    <Button
                        ...
                        disabled={!range?.from || !range?.to || isInCart || isAdded}
                        className="w-full py-3 text-lg flex items-center justify-center gap-2"
                    >
                        {isInCart ? 'Already in Cart' : isAdded ? <><FaCheck /> Added to Cart</> : 'Add to Cart'}
                    </Button>
                </div>
            </div>
        </div>
